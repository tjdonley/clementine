...
Cluster fitness uses the same signal weights as individual loops. A Hit for any loop updates both the loop AND its cluster alpha/beta. This lets sparse data propagate — a Hit for gaming-news partially informs all Content Discovery loops.
...
#### 7.1 Post-Session (Every Meta-Clem Run)

**Trigger:** User invokes Meta-Clem after a conversation session.
...
**Trigger:** End of week (or when the 7th daily log since last weekly exists). Run as part of a Meta-Clem invocation.
...
**Trigger:** End of month (or when 4 weekly consolidations exist since last monthly). Run as part of a Meta-Clem invocation.
...
|---------|---------|-------|-------------|-----------------|
| Post-session | Every Meta-Clem run | Latest daily log(s) | FACTS.md, STATE.md, MEMORY.md index, meta-report | 4 |
...
| Range | Label | Clem Behavior | Meta-Clem Action |
|---|---|---|---|
...
6. BELIEF: A belief Clementine holds, with its confidence level.
...
*Moved from STATE.md. Agent Clem doesn't need these during conversation — Meta-Clem uses them for scoring context.*
...
- **USER.md and MEMORY.md are shared** — Agent Clem updates during conversation, Meta-Clem updates during consolidation (Emotional Landscape, vividness scores, thematic clusters, profile refresh).
